...
class ACdkStack extends cdk.Stack {
  ...
  createPostgresCluster() {
    ...
    const tmpSeedsAssetDir = `${tmpLambdaAssetDir}/seeds`;
    fs.mkdirSync(tmpMigrationsAssetDir);
    fs.mkdirSync(tmpSeedsAssetDir);
    ...
    fs.readdirSync(DATABASES_PG_SEEDS_DIR).forEach((asset) =>
      fs.copyFileSync(
        `${DATABASES_PG_SEEDS_DIR}/${asset}`,
        `${tmpSeedsAssetDir}/${asset}`
      )
    );
