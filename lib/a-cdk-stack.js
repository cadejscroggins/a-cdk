...
const DATABASES_PG_MIGRATIONS_LAMBDA_DIR = `${__dirname}/assets/lambdas/pg-migrations`;
const DATABASES_PG_SEEDS_DIR = `${CWD}/src/databases/pg/seeds`;
...
class ACdkStack extends cdk.Stack {
  ...
  createPostgresCluster() {
    ...
    const tmpLambdaAssetDir = tmp.dirSync();
    fs.copySync(`${DATABASES_PG_MIGRATIONS_LAMBDA_DIR}/*`, tmpLambdaAssetDir);
    fs.copySync(DATABASES_PG_MIGRATIONS_DIR, tmpLambdaAssetDir);
    fs.copySync(DATABASES_PG_SEEDS_DIR, tmpLambdaAssetDir);

    const postgresMigrationsLambdaAsset = s3Assets.Asset(
      this,
    ...
    const postgresMigrationsLambda = new lambda.Function(
      ...
      {
        code: lambda.Code.fromBucket(postgresMigrationsLambdaAsset, 'index.js'),
        environment: {
          clusterDatabaseName: databaseName,
          clusterEndpoint: postgresCluster.clusterEndpoint,
          clusterSecretArn: postgresCluster.secret.secretFullArn,
        },
    ...
    new customResources.Provider(
      this,
      formatResourceName(this.artifactId, 'PostgresMigrationsCustomResource'),
      {
