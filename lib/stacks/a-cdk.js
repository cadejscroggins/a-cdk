...
class ACdkStack extends cdk.Stack {
  ...
  async createPostgresMigrationsCustomResource({ postgresCluster }) {
    ...
    const postgresMigrationsLambda = new lambda.Function(this, lambdaId, {
      code: this.createLambdaCodeAsset({
        assets: [C.PATH.DATABASES_PG_MIGRATIONS_LAMBDA_MIGRATIONS_DIR],
      ...
      environment: {
        ...
        [C.ENV_VAR_KEY.PG_CLUSTER_MIGRATIONS_LAMBDA_MIGRATIONS_DIR]:
          C.PATH.DATABASES_PG_MIGRATIONS_LAMBDA_MIGRATIONS_DIR,
  ...
  createLambdaCodeAsset({ assets = [], entry, lambdaId }) {
    ...
    if (assets.length) {
      assets.forEach((a) => fse.copySync(a, tmpLambdaAssetDir));
    }
