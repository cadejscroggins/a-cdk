...
class ACdkStack extends cdk.Stack {
  ...
  attachLambdaPermissions({ lambdas, postgresCluster, userPool }) {
    ...
    Object.entries(lambdas).forEach(([name, lambdaResource]) => {
      ...
      const lambdaSnsActions = lambdaActions?.sns || [];
      const statements = [];
      ...
      if (lambdaSnsActions.length) {
        statements.push(
          new iam.PolicyStatement({
            actions: lambdaSnsActions.map((a) => `${C.ACTION_PREFIX.SNS}${a}`),
            resources: ['*'],
          })
        );
      }
  ...
  createUserPool({ lambdas }) {
    ...
    const userPool = new cognito.UserPool(this, userPoolId, {
      ...
      lambdaTriggers: {
        createAuthChallenge: lambdas[C.AUTH_TRIGGER.CREATE_AUTH_CHALLENGE],
        customMessage: lambdas[C.AUTH_TRIGGER.CUSTOM_MESSAGE],
        defineAuthChallenge: lambdas[C.AUTH_TRIGGER.DEFINE_AUTH_CHALLENGE],
        preSignUp: lambdas[C.AUTH_TRIGGER.PRE_SIGNUP],
        verifyAuthChallengeResponse: lambdas[C.AUTH_TRIGGER.VERIFY_AUTH_CHALLENGE_RESPONSE],
      },
  ...
  getApiDataSourceFromTypeAndName({
    ...
    switch (templateDataSourceType) {
      ...
      default: {
        if (defaultToNone) dataSource = noneDataSource;
      }
    }
