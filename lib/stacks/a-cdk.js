...
class ACdkStack extends cdk.Stack {
    ...
    return Object.entries(
      fs
        ...
        .reduce((acc, mappingTemplateFile) => {
          ...
          const dataSourceId = formatResourceId(
            {
              [C.DATA_SOURCE_TYPE.DYNAMODB]: C.ID_PART.DYNAMODB,
              [C.DATA_SOURCE_TYPE.POSTGRES]: C.ID_PART.POSTGRES,
            }[templateDataSourceType],
            templateDataSourceName,
            C.ID_PART.DATA_SOURCE
          );
  ...
  async createPostgresCluster() {
    ...
    await this.createPostgresMigrationsCustomResource({
      databaseId,
      postgresCluster,
    });
  ...
  async createPostgresMigrationsCustomResource({
    databaseId,
    postgresCluster,
  }) {
    ...
    const postgresMigrationsLambda = new lambda.Function(
      ...
      migrationsLambdaId,
      {
        ...
        environment: {
          ...
          migrationsDir: C.PATH.DATABASES_PG_MIGRATIONS_LAMBDA_MIGRATIONS_DIR,
          migrationsTableName: 'a_cdk_migrations',
    ...
    const postgresMigrationsCustomResource = new customResources.Provider(
      this,
      formatResourceId(migrationsLambdaId, C.ID_PART.CUSTOM_RESOURCE_PROVIDER),
      {
    ...
    new cdk.CustomResource(
      this,
      formatResourceId(migrationsLambdaId, C.ID_PART.CUSTOM_RESOURCE),
      {
    ...
    postgresMigrationsLambda.role.node.defaultChild.overrideLogicalId(
      formatResourceId(migrationsLambdaId, C.ID_PART.SERVICE_ROLE)
    );
