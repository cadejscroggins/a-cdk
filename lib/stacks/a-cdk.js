...
class ACdkStack extends cdk.Stack {
    ...
    Object.entries(lambdas).forEach(([name, lambdaResource]) => {
      ...
      if (statements.length) {
        lambdaResource.role.attachInlinePolicy(
          new iam.Policy(
            this,
            formatResourceId(lambdaResource.physicalName, C.ID_SUFFIX.POLICY),
            { statements }
          )
        );
      }
  ...
  createApi({ dynamodbTables, postgresCluster }) {
    const apiId = formatResourceId(this.artifactId, C.ID_SUFFIX.API);
    ...
    api.addNoneDataSource(formatResourceId(C.DATA_SOURCE_TYPE.NONE));
  ...
  createApiDynamodbDataSources({ api, dynamodbTables }) {
    ...
    return fs.readdirSync(C.PATH.DATABASES_DDB_DIR).reduce((acc, fileName) => {
      ...
      const dataSourceId = formatResourceId(
        C.DATA_SOURCE_TYPE.DYNAMODB,
        tableName,
        C.ID_SUFFIX.DATA_SOURCE
      );
  ...
  async createPostgresCluster() {
    ...
    const clusterId = formatResourceId(this.artifactId, C.ID_SUFFIX.PG_CLUSTER);
    const vpcId = formatResourceId(this.artifactId, C.ID_SUFFIX.PG_VPC);
  ...
  async createPostgresMigrationsCustomResource({ postgresCluster }) {
    const migrationsLambdaId = formatResourceId(
      this.artifactId,
      C.ID_SUFFIX.PG_MIGRATIONS
    );

    const migrationsCustomResourceId = formatResourceId(
      this.artifactId,
      C.ID_SUFFIX.PG_MIGRATIONS_CUSTOM_RESOURCE
    );
    ...
    new customResources.Provider(this, migrationsCustomResourceId, {
  ...
  createPostgresMigrationsCustomResourceLambdaAsset() {
    const migrationsLambdaAssetId = formatResourceId(
      this.artifactId,
      C.ID_SUFFIX.PG_MIGRATIONS_LAMBDA_ASSET
    );
  ...
  createIdentityPoolRole({ api, identityPool, roleType }) {
    ...
    const role = new iam.Role(
      this,
      formatResourceId(identityPool.identityPoolName, roleType, 'role'),
      {
  ...
  createUserPool({ lambdas }) {
    const authConf = this.node.tryGetContext(C.CONTEXT_KEY.AUTH);
    const userPoolId = formatResourceId(this.artifactId, C.ID_SUFFIX.USERS);
  ...
  createUserPoolClient({ userPool }) {
    const userPoolClientId = formatResourceId(
      this.artifactId,
      C.ID_SUFFIX.WEB_CLIENT
    );

    return new cognito.UserPoolClient(this, userPoolClientId, {
      userPool,
