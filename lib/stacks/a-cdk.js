...
class ACdkStack extends cdk.Stack {
  ...
  createApi({ dynamodbTables, lambdas, postgresCluster }) {
    ...
    this.createApiResolvers({
      ...
      lambdaDataSources: this.createApiLambdaDataSources({
        api,
        lambdas,
      }),
  ...
  createApiPipelineFunctions({ api }) {
    ...
    return Object.entries(
          ...
          const dataSourceId = formatResourceId(
            {
              ...
              [C.DATA_SOURCE_TYPE.LAMBDA]: C.ID_PART.LAMBDA,
      ...
      // TODO: change to appsync.AppsyncFunction
      ...
      webpack(
        {
          ...
          resolveLoader: {
            modules: [C.PATH.A_CDK_NODE_MODULES_DIR],
          },
          target: 'node',
        },
        (err, stats) => {
